document.addEventListener('DOMContentLoaded', function() {
    ...
    function runAGC(keyword) {
        ...
        document.title = seoTitle + ' | NiceNails';
        if(detailTitle) detailTitle.textContent = seoTitle;
        
        // Render Gambar Utama dan Tombol Pinterest
        if(mainImageWrapper) {
            const mainQueryImg = keyword + " nails pinterest";
        ...
        // Fungsi Render Grid (Tanpa Download Button)
            ...
            const html = dataArray.map(data => {
                ...
                <div class="wallpaper-card">
                    <a href="${linkUrl}" class="img-link" title="${capitalize(data.kw)} Nails">
                        <img src="${data.img}" alt="${capitalize(data.kw)} Nail Art" loading="lazy">
                    </a>
                </div>
                `;
            }).join('');
        ...
        function fetchRelated() {
            return new Promise(resolve => {
                ...
                // Ganti kueri google suggest ke "nails"
        ...
        async function buildContent() {
            ...
            function getUniqueData(kwString) {
                // Filter keyword dari kata 'wallpaper' / 'nails' agar bersih
                let cleanKw = kwString.replace(/wallpaper|nails/gi, '').trim();
                if(!cleanKw) cleanKw = kwString;
                
                // Set rasio gambar w=1080&h=1620 (2:3)
                let queryImage = cleanKw + " aesthetic nails";
                let imageUrl = `https://tse1.mm.bing.net/th?q=${encodeURIComponent(queryImage)}&w=1080&h=1620&c=7&rs=1&p=0`;
